...
function SortablePaginate({ columns, url, id }) {
  ...
  function handleDragEnd(event) {
    ...
    if (active.id !== over.id) {
      queryClient.setQueryData([url], (old) => {
        const oldIndex = old.data.indexOf(active.id);
        const newIndex = old.data.indexOf(over.id);

        return {
          ...old,
          data: arrayMove(old.data, oldIndex, newIndex),
        };
      });
    }
  }
      ...
      <div className="d-flex justify-content-between mt-2 mt-md-0 flex-wrap gap-2">
        ...
        <ul className="pagination">
          ...
          <For
            ...
            render={(item) => (
              <li
                onClick={useCallback(() => setPage(item), [page])}
...
export default memo(SortablePaginate);
